...
export class Client {
  ...
  send(
    command: string,
    ignoreErrorCodesDEPRECATED = false
  ): Promise<FTPResponse> {
    if (ignoreErrorCodesDEPRECATED) {
      // Deprecated starting from 3.9.0
      this.ftp.log(
        "Deprecated call using send(command, flag) with boolean flag to ignore errors. Use sendIgnoringError(command)."
      );
  ...
  async useDefaultSettings(): Promise<void> {
    ...
    ); // Make sure MLSD listings include all we can parse
    if (this.ftp.hasTLS) {
  ...
  async size(path: string): Promise<number> {
    ...
    if (Number.isNaN(size)) {
      throw new Error(
        `Can't parse response to command '${command}' as a numerical value: ${res.message}`
      );
    }
    return size;
  }
   ...
   * @protected
   */
  protected async _uploadWithCommand(
    source: Readable | string,
    remotePath: string,
   ...
   * @protected
   */
  protected async _uploadLocalFile(
    localPath: string,
    remotePath: string,
    command: UploadCommand,
    options: UploadOptions
   ...
   * @protected
   */
  protected async _uploadFromStream(
    source: Readable,
    remotePath: string,
    command: UploadCommand
  ): Promise<FTPResponse> {
   ...
   * @protected
   */
  protected async _downloadToFile(
    localPath: string,
    remotePath: string,
    startAt: number
  ) {
   ...
   * @protected
   */
  protected async _downloadToStream(
   ...
   * @protected
   */
  protected async _requestListWithCommand(
    command: string
  ): Promise<FileInfo[]> {
  ...
  protected async _exitAtCurrentDirectory<T>(
    func: () => Promise<T>
  ): Promise<T> {
  ...
  async uploadDir(localDirPath: string, remoteDirPath?: string): Promise<void> {
    this.ftp.log(
      "Warning: uploadDir() has been deprecated, use uploadFromDir()."
    );
  ...
  async downloadDir(localDirPath: string): Promise<void> {
    this.ftp.log(
      "Warning: downloadDir() has been deprecated, use downloadToDir()."
    );
